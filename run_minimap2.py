...
'''
Align reads against input assembly using Minimap2
...
def main():
    '''Main function'''
    argparse_usage = (
        'run_minimap2.py -r <read_files> -a <asm_file> -o <output_dir>')
    ...
    print('INFO: Aligning reads to reference genome using minimap2')
    d_read_files = organize_read_files(read_files)
    align_reads_minimap2(d_read_files, asm_file, output_dir)
    print('INFO: Minimap2 done')
...
def align_reads_minimap2(d_qced_read_files, asm_file, output_dir):
    ...
    for read_file_prefix, r_files in d_qced_read_files.items():
        ...
        if os.path.exists(bam_out):
            return
        # Command: minimap2 -a reference.fa reads.fq | samtools view -bSF4 - | \
        # samtools sort -o output.bam
        command1 = [
            'minimap2', '-ax', 'sr', asm_file, r_files[0], r_files[1]]
        ...
        log_file = os.path.join(output_dir, 'minimap2.log')
        log_handle_minimap2 = open(log_file, 'w')
        p1 = subprocess.Popen(
            command1, stdout=subprocess.PIPE, stderr=log_handle_minimap2)
        ...
        if p3.returncode != 0:
            print('ERROR: Minimap2 failed. Please check minimap2.log')
            sys.exit(1)
